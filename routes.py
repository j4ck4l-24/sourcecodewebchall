...
@requires_teacher
def flag():
    return render_template('flag.html', flag="InfosecIITR{FAKE_FLAG}", is_auth=True, is_teacher_role=True)
#this is a fake flag. the real flag is on the server
@api.route('/logout')
def logout():
...
def download_src():
    ...
    return render_template('src.txt')